...
const Blogs = ({ blogs, categories, tags, totalBlogs, blogsLimit, blogSkip, router }) => {
    ...
    const loadMore = () => {
        ...
        listBlogsCategoriesTags(toSkip, limit).then(data => {
            if (data.error) {
    ...
    const loadMoreButton = () => {
        return (
            size > 0 &&
            size >= limit && (
                <button onClick={loadMore} className="btn btn-outline-primary btn-lg block">
                  SHOW MORE BLOGS
                </button>
            )
        );
...
Blogs.getInitialProps = () => {
    let skip = 0;
    let limit = 2;
    return listBlogsCategoriesTags(skip, limit).then(data => {
        if (data.error) {
            console.log(data.error);
        } else {
            return {
